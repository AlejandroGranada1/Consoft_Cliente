...
export default function PedidoDetallePage() {
  ...
  if (isLoading)
    return <p className="p-6 text-center">Cargando pedido...</p>

  if (error || !pedido)
    return <p className="p-6 text-center">No existe el pedido</p>

  const productos = pedido.raw?.items ?? []   // <-- AQUÍ EL FIX
          ...
          {productos.map((prod: any) => (
            ...
            >
              {prod.id_servicio?.image ? (
                <img
                  src={prod.id_servicio.image}
                  alt={prod.id_servicio.name}
              ...
              <p className="font-medium text-gray-800">
                {prod.id_servicio?.name || "Servicio"}
              </p>
            </div>
          ))}
        </div>
